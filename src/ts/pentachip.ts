...
export default class Pentachip extends Renderable {
        ...
        opponentChips.map(
            (opponentChip) => {
                for (const chip of chips) {
                    if (opponentChip.position.x === chip.x
    ...
    private getChips(ownedBy: type.PlayerIndex): type.GameChipInterface[] {
        return this.chips
            .filter((chip) => chip.ownedBy === ownedBy);
    }
    ...
    private hasPotential(chips: type.GameChipInterface[]): boolean {
        ...
        chips.map((chip) => {
            potentialList = potentialList.concat(this.searchHintPoints(chip.position));
        });
    ...
    private checkPoked() {
        ...
        selfChips.map((chip) => {
            const isPoked = this.checkSingleAround(chip.position, opponentChips, "poke");
            if (isPoked) {
    ...
    private checkSingleAround(
        ...
        action: "clutch" | "poke" = "clutch"): boolean {
    ...
    private isOccupied(positionList: type.GameChipPosition[], ...testPoints: type.GameChipPosition[]): boolean {
